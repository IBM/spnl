...
git clone https://github.com/${VLLM_ORG:-starpit}/${VLLM_REPO:-vllm-ibm}.git vllm -b ${VLLM_BRANCH:-spnl-ibm-2}
cd vllm
uv venv --seed
...
wait $spnl_pid # wait for the spnl CLI build to finish
uv pip install maturin
maturin develop --release -F tok,run_py -m spnl/Cargo.toml
...
then
    ...
    find "$TESTS_DIR" -type f -name "*.sh" -exec bash -c 'for script; do echo "Executing: $script"; "$script"; done' _ {} +
